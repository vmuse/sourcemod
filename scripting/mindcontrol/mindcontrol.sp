...
#define PLUGIN_NAME     "TF2 Mind Control"
#define PLUGIN_AUTHOR   "https://github.com/vmuse"
#define PLUGIN_CONTACT  "https://github.com/vmuse/sourcemod"
#define PLUGIN_DESCRIP  "Allows admins to take control of other players and apply manual or AI control"
...
**/
                ...
                if(buttons & IN_USE)    // use will lock onto any valid player (doens't check teams here)
                {
                    ...
                    if(cooldown[g_MindControlOwner[client]] < time)
                    {
                        ...
                        else if((g_MindControlTarget[g_MindControlOwner[client]] = FindValidEnemy(client, g_MindControlOwner[client], origin))) 
                        {
                            cooldown[g_MindControlOwner[client]] = time + 1.0;
 
                            automate[g_MindControlOwner[client]] = true;
                            PrintCenterText(g_MindControlOwner[client], "AutoLock On: %N", g_MindControlTarget[g_MindControlOwner[client]]);
                        }
                    }
                }
...
FindValidEnemy(client, owner, Float:origin[3])
{
    new target = TraceToTarget(client);
    ...
    decl Float:distance;
    decl Float:pos[3];
    new Float:bestdistance = 900000000.0;
    new besttarget;

    for(target=1; target<=MaxClients; target++)
    {
        if(owner != target && !g_MindControlOwner[target] && IsClientInGame(target) && IsPlayerAlive(target) && GetClientTeam(target) != team)
        {
